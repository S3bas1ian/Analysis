...
void edge_hit_xy_tgraph(std::string path, std::string particle)
{
    ...
    auto graph1_phantom = new TGraph(x1_phantom, y1_phantom);
    graph1_phantom->SetTitle("detector 0; x [mm]; y [mm]");
    auto graph2_phantom = new TGraph(x2_phantom, y2_phantom);
    graph2_phantom->SetTitle("detector 1; x [mm]; y [mm]");
    auto graph3_phantom = new TGraph(x3_phantom, y3_phantom);
    graph3_phantom->SetTitle("detector 2; x [mm]; y [mm]");
    auto graph4_phantom = new TGraph(x4_phantom, y4_phantom);
